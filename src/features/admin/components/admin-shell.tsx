...
const pageMeta = [
  ["/admin/billing/new", "Create billing document", "Build, review, and issue a client document."],
  ["/admin/billing/", "Billing document", "Review financial details, history, and payment state."],
...
export function AdminShell({
  ...
  const visibleNavigation = navigation.filter((item) => !item.founderOnly || user.role === "founder_admin");
    ...
    <div className="min-h-screen bg-[#f4f7fb] text-slate-950">
      ...
      >
        ...
        <div className="px-4 pt-5">
          <Link href="/admin/billing/new" onClick={() => setMobileOpen(false)} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-ktf-blue px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-ktf-blue-deep">
            <FilePlus2 className="h-4 w-4" /> Create document
          </Link>
        </div>
        ...
        <div className="border-t border-slate-100 p-4">
          <form action="/admin/api/auth/logout" method="post">
            <button className="flex h-10 w-full items-center gap-3 rounded-xl px-3 text-sm font-medium text-slate-500 transition hover:bg-slate-50 hover:text-slate-950">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200/90 bg-white/95 backdrop-blur lg:ml-[248px]">
        <div className="flex min-h-[86px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            <button type="button" className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-600 lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open navigation">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Link href="/admin/search" className="hidden h-10 items-center gap-2 rounded-xl border border-slate-200 px-3 text-sm text-slate-500 xl:flex">
