...
import { DEFAULT_PAYMENT_TERMS, invoiceDateWarnings, progressFigures, progressHeading, progressStatement, termsForBalance } from "./document-copy";
...
export function BillingEditor({ clients, projects, settings, initialDocument, initialDraft }: {
  ...
  const [previouslyInvoiced, setPreviouslyInvoiced] = useState(draftString(initialDraft, "previouslyInvoiced", initialDocument?.previouslyInvoiced ? String(initialDocument.previouslyInvoiced) : ""));
  ...
  const progress = progressBilling
    ? calculateProgressSummary({ contractValue: Number(contractValue) || 0, previouslyInvoiced: Number(previouslyInvoiced) || 0 }, totals)
    : null;
  ...
  function payload() {
    return { type, customTypeLabel, clientChoice, newClientName, projectChoice, newProjectName, issueDate, dueDate, currency, items, notes, terms: effectiveTerms, paymentInstructions, purchaseOrder, valueLabel, contractValue, previouslyInvoiced, frequency, nextRunDate, autoIssue };
  }
    ...
    try {
      const response = await fetch(initialDocument ? `/admin/api/billing/${initialDocument.id}` : "/admin/api/billing", {
        ...
        body: JSON.stringify({
          ...
          previouslyInvoiced: progressBilling && Number(previouslyInvoiced) > 0 ? Number(previouslyInvoiced) : undefined,
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(430px,0.95fr)]">
      <div className="space-y-5">
        <section className="rounded-lg border border-ktf-gray-200 bg-white p-5 shadow-card sm:p-6">
          ...
          {progressBilling && <div className="mt-5 grid gap-4 rounded-lg border border-ktf-blue/15 bg-ktf-blue/5 p-4 sm:grid-cols-2"><label><span className={labelClass}>Total engagement value ({currency})</span><input className={inputClass} type="number" min="0" step="0.01" value={contractValue} onChange={(event) => setContractValue(event.target.value)} placeholder="Total agreed value of the whole engagement" /></label><label><span className={labelClass}>Invoiced earlier ({currency})</span><input className={inputClass} type="number" min="0" step="0.01" value={previouslyInvoiced} onChange={(event) => setPreviouslyInvoiced(event.target.value)} placeholder="0" /></label><p className={`text-xs leading-5 sm:col-span-2 ${progress?.overBilled ? "font-semibold text-amber-800" : "text-ktf-gray-600"}`}>{progress ? progressStatement(progress, currency) : "Enter the total engagement value and this invoice will state what it covers and what remains."}</p></div>}
        </section>
