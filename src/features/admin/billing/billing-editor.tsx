...
import { DEFAULT_PAYMENT_TERMS, invoiceDateWarnings, termsForBalance } from "./document-copy";
import { BILLING_DOCUMENT_TYPE_OPTIONS, billingDocumentTypeLabel } from "./document-types";
import { addDays, calculateDocumentTotals, calculateLine, formatMoney, toIsoDate } from "./money";
...
export function BillingEditor({ clients, projects, settings, initialDocument, initialDraft }: {
  ...
  const displayType = billingDocumentTypeLabel(type, customTypeLabel);
  ...
  function payload() {
    return { type, customTypeLabel, clientChoice, newClientName, projectChoice, newProjectName, issueDate, dueDate, currency, items, notes, terms: effectiveTerms, paymentInstructions, purchaseOrder, valueLabel, frequency, nextRunDate, autoIssue };
  }
