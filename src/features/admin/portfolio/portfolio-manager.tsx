...
function PortfolioCard({ project }: { project: PortfolioProject }) {
  ...
  return (
    <article className="bt-card-motion overflow-hidden rounded-lg border border-slate-200 bg-white">
      ...
      <div className="p-4">
        ...
        <div className="mt-4 flex items-center gap-2 border-t border-slate-100 pt-3">
          <details className="group/edit flex-1">
            <summary className={`${secondaryButtonClass} w-full cursor-pointer list-none`}><Pencil className="h-3.5 w-3.5" /> Edit</summary>
            <div className="fixed inset-0 z-[70] overflow-y-auto bg-slate-950/55 p-3 sm:p-8">
              <div className="mx-auto max-w-3xl rounded-lg bg-white p-5 shadow-2xl sm:p-7">
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div><h3 className="font-bold text-slate-950">Edit {project.name}</h3><p className="mt-1 text-xs text-slate-500">Changes update the public portfolio after saving.</p></div>
                  <button type="button" onClick={(event) => event.currentTarget.closest("details")?.removeAttribute("open")} className="cursor-pointer rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600">Close</button>
                </div>
...
function PortfolioForm({ project }: { project?: PortfolioProject }) {
  const router = useRouter();
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!project) event.currentTarget.reset();
      router.refresh();
    } catch (caught) {
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label={project ? "Replace project image" : "Project image"} hint="PNG, JPEG, or WebP. Maximum 8 MB.">
          <label className="flex h-10 cursor-pointer items-center gap-2 rounded-lg border border-dashed border-slate-300 px-3 text-xs font-semibold text-slate-600 hover:border-ktf-blue">
            <ImageUp className="h-4 w-4" /><span>{project ? "Choose a new image (optional)" : "Choose finished-project image"}</span>
...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return <label><span className={labelClass}>{label}</span>{children}{hint && <span className="mt-1 block text-[10px] leading-4 text-slate-400">{hint}</span>}</label>;
}
