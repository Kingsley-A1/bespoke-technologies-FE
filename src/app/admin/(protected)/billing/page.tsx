...
import { BILLING_DOCUMENT_TYPE_OPTIONS, billingDocumentTypeLabel, isPayableBillingType } from "@/features/admin/billing/document-types";
...
import { listInvoiceDrafts } from "@/features/admin/billing/drafts";
...
export default async function BillingPage({ searchParams }: { searchParams: Promise<{ q?: string; type?: string; status?: string; page?: string }> }) {
  ...
  const incompleteDrafts = await listInvoiceDrafts(session);
  ...
  const issued = snapshot.documents.filter((document) => isPayableBillingType(document.type) && !["draft", "pending_approval", "approved", "voided"].includes(document.status) && document.currency === "NGN");
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><p className="max-w-2xl text-sm leading-6 text-slate-500">Create consistent invoices, reconcile delivered invoices, and keep recurring work reliable.</p><div className="flex flex-wrap gap-2"><form action={runRecurringAction}><button className="inline-flex h-10 items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 text-xs font-semibold text-slate-600"><Play className="h-4 w-4" /> Run due schedules</button></form><Link href="/admin/billing/new" className="inline-flex h-10 items-center gap-2 rounded-lg bg-ktf-blue px-4 text-xs font-semibold text-white"><FilePlus2 className="h-4 w-4" /> New invoice</Link></div></section>
      ...
      {incompleteDrafts.length > 0 && <Panel><PanelHeader title="Incomplete invoice drafts" description="Server-saved work you can continue from any signed-in device." /><div className="grid gap-px bg-ktf-gray-200 sm:grid-cols-2 xl:grid-cols-3">{incompleteDrafts.map((draft) => <Link key={draft.id} href={`/admin/billing/new?draft=${draft.id}`} className="bt-card-motion bg-white p-5"><p className="text-sm font-bold text-ktf-navy">{draft.title}</p><p className="mt-2 text-[11px] text-ktf-gray-500">Saved {formatAdminDate(draft.updatedAt)}</p><span className="mt-4 inline-flex text-xs font-semibold text-ktf-blue">Continue invoice →</span></Link>)}</div></Panel>}

      <Panel>
        <PanelHeader title="Invoice register" description="Searchable, filtered, server-authoritative invoice history." />
        <form action="/admin/billing" className="grid gap-3 border-b border-slate-100 p-4 sm:grid-cols-[1fr_190px_190px_auto]"><label className="relative"><span className="sr-only">Search billing</span><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-slate-400" /><input className={`${inputClass} pl-9`} name="q" defaultValue={filters.q} placeholder="Number, client, or email" /></label><select className={inputClass} name="type" defaultValue={type}><option value="all">All types</option>{BILLING_DOCUMENT_TYPE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><select className={inputClass} name="status" defaultValue={status}><option value="all">All states</option><option value="draft">Draft</option><option value="pending_approval">Pending approval</option><option value="approved">Approved</option><option value="sent">Sent</option><option value="partially_paid">Partially paid</option><option value="paid">Paid</option><option value="overdue">Overdue</option><option value="voided">Voided</option><option value="active">Active schedule</option><option value="paused">Paused schedule</option><option value="failed">Failed schedule</option></select><button className="h-10 rounded-lg border border-slate-200 px-4 text-xs font-semibold text-slate-600">Apply filters</button></form>
        <div className="overflow-x-auto"><table className="w-full min-w-[960px] text-left"><thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500"><tr><th className="px-6 py-3">Invoice</th><th className="px-4 py-3">Client</th><th className="px-4 py-3">Issued</th><th className="px-4 py-3">Total</th><th className="px-4 py-3">Balance</th><th className="px-4 py-3">Status</th><th className="px-6 py-3 text-right">Open</th></tr></thead><tbody className="divide-y divide-slate-100">{documents.map((document) => { const totals = calculateDocumentTotals(document, snapshot.payments); return <tr key={document.id} className="text-sm hover:bg-slate-50/70"><td className="px-6 py-4"><p className="font-semibold text-slate-900">{document.documentNumber}</p><p className="mt-1 text-[11px] text-slate-400">{billingDocumentTypeLabel(document.type, document.customTypeLabel)}{document.recurrence ? ` · ${document.recurrence.frequency}` : ""}{document.revision > 1 ? ` · revision ${document.revision}` : ""}</p></td><td className="px-4 py-4"><p className="font-medium text-slate-700">{document.client.name}</p><p className="mt-1 text-xs text-slate-400">{document.client.email}</p></td><td className="px-4 py-4 text-xs text-slate-500">{formatAdminDate(document.issueDate)}</td><td className="px-4 py-4 font-semibold text-slate-800">{formatMoney(totals.total, document.currency)}</td><td className="px-4 py-4 font-semibold text-slate-800">{document.valueLabel || formatMoney(totals.balance, document.currency)}</td><td className="px-4 py-4"><StatusPill value={document.recurrence?.state ?? document.status} /></td><td className="px-6 py-4 text-right"><Link href={`/admin/billing/${document.id}`} className="inline-flex h-9 items-center rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-600">Review</Link></td></tr>; })}</tbody></table></div>
        <div className="flex items-center justify-between border-t border-slate-100 px-5 py-4 text-xs text-slate-500"><span>{matching.length} matching invoices · page {Math.min(page, totalPages)} of {totalPages}</span><div className="flex gap-2">{page > 1 && <Link className="rounded-lg border border-slate-200 px-3 py-2 font-semibold" href={pageHref(page - 1)}>Previous</Link>}{page < totalPages && <Link className="rounded-lg border border-slate-200 px-3 py-2 font-semibold" href={pageHref(page + 1)}>Next</Link>}</div></div>
      </Panel>
