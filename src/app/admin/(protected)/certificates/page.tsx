...
export default async function CertificatesPage() {
  ...
  const [snapshot, certificates] = await Promise.all([getAdminSnapshot(), listOwnershipCertificates()]);
  const activeProjectIds = new Set(certificates.filter(item => item.status !== "revoked").map(item => item.projectId));
  const readyProjects = snapshot.projects.flatMap(project => {
